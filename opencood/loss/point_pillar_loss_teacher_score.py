...
class PointPillarLossTeacherScore(nn.Module):
    def __init__(self, args):
        super(PointPillarLossTeacherScore, self).__init__()
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        # cls loss
        cls_preds = output_dict[f'teacher_cls_preds{suffix}'].permute(0, 2, 3, 1).contiguous() \
                    .view(batch_size, -1,  1)
        ...
        reg_preds = output_dict[f'teacher_reg_preds{suffix}'].permute(0, 2, 3, 1).contiguous().view(batch_size, -1, 7)
        ...
        if self.dir:
            ...
            dir_logits = output_dict[f"teacher_dir_preds{suffix}"].permute(0, 2, 3, 1).contiguous().view(batch_size, -1, 2) # [N, H*W*#anchor, 2]
