    ...
    def forward(self, x, record_len, pairwise_t_matrix):
        ...
        for b in range(B):
            ...
            neighbor_feature = warp_affine_simple(batch_node_features[b],
                                                  t_matrix[i, :, :, :],
                                                  (H, W))
            ego_feature = neighbor_feature[0].view(1, C, H, W)
            out.append(ego_feature[0])
        out = torch.stack(out)
...
class DiscoFusion(nn.Module):
    def __init__(self, feature_dims):
        ...
        self.fc = nn.Sequential(
            nn.Linear(feature_dims, feature_dims // self.fcScale, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(feature_dims // self.fcScale, feature_dims, bias=False),
            nn.Sigmoid()
        )
    ...
    def forward(self, xx, record_len, pairwise_t_matrix):
        ...
        for b in range(B):
            ...
            agent_weight = agent_weight * channel_weight.expand_as(agent_weight)
            # -------------------------------------ChannelAttention-----------------------------------
            # (N, C, H, W)
